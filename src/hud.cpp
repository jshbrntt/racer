...
HUD::HUD()
{
	...
	entities.push_back(Entity(goBox,	  goBox,	  center, 0, PNGtoGLtexture("textures/text/GO.png")));
	entities.push_back(Entity(oneBox,	  oneBox,	  center, 0, PNGtoGLtexture("textures/text/1.png")));
	entities.push_back(Entity(twoBox,	  twoBox,	  center, 0, PNGtoGLtexture("textures/text/2.png")));
	entities.push_back(Entity(threeBox,	  threeBox,	  center, 0, PNGtoGLtexture("textures/text/3.png")));
	entities.push_back(Entity(youWinBox,  youWinBox,  center, 0, PNGtoGLtexture("textures/text/YOU_WIN.png")));
	entities.push_back(Entity(youLoseBox, youLoseBox, center, 0, PNGtoGLtexture("textures/text/YOU_LOSE.png")));
	entities.push_back(Entity(pausedBox,  pausedBox,  center, 0, PNGtoGLtexture("textures/text/PAUSED.png")));
...
void HUD::draw()
{
	...
	if(!pause)
	{
		...
		if(raceWon == 0)
		{
			if(player->currentLap == lapLimit)	raceWon	= 1;
			if(enemy->currentLap == lapLimit)	raceWon	= 2;
		}

		// YOU WIN!:
		if(raceWon == 1)	entities[4].draw();

		// YOU LOSE!:
		if(raceWon == 2)	entities[5].draw();
