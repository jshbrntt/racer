...
GLuint Enemy::enemyTexture()
{
	// IMPORTING PLAYER TEXTURE:
	return PNGtoGLtexture("textures/cars/green_car.png");
}
...
Enemy::Enemy(Point position, float rotation):
Car(enemyVerts(), enemyShape(), position, rotation, enemyTexture())
{
...
void Enemy::runAI()
{
	...
	float desiredAngle = atan2((tileEnt.position.y + (track->tileSize / 2) - position.y),
							   (tileEnt.position.x + (track->tileSize / 2) - position.x)) * (180 / M_PI);
	
	// IF ROTATION IS LARGER THAN A FULL TURN:
	if(abs(angle - desiredAngle) >= 180)
	{
		// ADJUST THE DESIRED ANGLE:
		if(angle < 0)	desiredAngle -= 360; 
		if(angle > 0)	desiredAngle += 360;
	}

	// ROTATE THE CAR TOWARDS THE TILE:
	if(angle < (desiredAngle + (rotationDeadzone / 2)))	turnLeft();
	if(angle > (desiredAngle - (rotationDeadzone / 2)))	turnRight();
