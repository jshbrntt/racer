...
Entity::Entity(vector<Point> verts, vector<Point> shape, Point position, float angle, GLuint texture)
{
	// PARSING PARAMETERS:
	this->verts		= verts;
	this->shape		= shape;
	...
	if(verts.size() >= 3)
	{
		...
		for(int i = 0; i != verts.size(); i++)
		{
			...
			if(texture != 0)
			{
				...
				if(vert.x > 0) texturePosition.x = 1;
				if(vert.y > 0) texturePosition.y = 1;
