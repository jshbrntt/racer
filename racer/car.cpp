...
Car::Car():Entity(){}
Car::Car(vector<Point> verts, vector<Point> shape, Point position, float angle, GLuint texture):
Entity(verts, shape, position, angle, texture)
{
	...
	GLuint flameTexture = PNGtoGLtexture("textures/fx/flame.png");
...
void Car::update()
{
	...
	if(currentLap == lapLimit)	immobilized = true;
	...
	if(turboOn)
	{
		...
		if(forward) flameDist += ((abs(linearVelocity.x) + abs(linearVelocity.y)) / 6);
...
void Car::draw(Point parentPosition)
{
	   ...
	   (relative.y >= offset - track->tileSize  && relative.y <= S_HEIGHT + track->tileSize - offset))
	{
		...
		if(turboOn)	flame.draw(parentPosition);
...
void Car::checkCollisions()
{
	...
	for(int i = 0; i != proximityTiles.size(); i++)
	{
		...
		if(tileIndex.x < 0)				continue;
		if(tileIndex.x > mapWidth - 1)	continue;
		if(tileIndex.y < 0)				continue;
		if(tileIndex.y > mapHeight - 1)	continue;
